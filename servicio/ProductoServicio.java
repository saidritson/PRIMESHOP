...
public class ProductoServicio {
    ...
    public List<ProductoDto> listarProductos() {
        return productoRepo.findAllActivosConTallas().stream().map(p -> {
            ...
            dto.setPrecio(p.getPrecio());
            dto.setColor(p.getColor());
            dto.setImagen(p.getImagenUrl()); // O p.getImagen() según tu entidad
            if (p.getCategoria() != null) dto.setCategoriaNombre(p.getCategoria().getNombre());
            return dto;
        }).collect(Collectors.toList());
    }

    public ProductoDto obtenerDetalle(int id) {
